...
def conclusion(s, skip):
    n = search_all_empty(s)
    p = search_empty_pos(s, skip)
    print(f"n={n}, p={p}, skip={skip}")
    val = search_possible_values(get_row(s, p), get_column(s, p), get_block(s, p))
    print(f"n={n}, p={p}, val={val}, skip={skip}")
    if len(val) == 1:
        write_value(s, p, val[0])
        print_sudoku(s)
        if n > skip + 1:
            conclusion(s, skip)
        elif n == 1:
